   ...
   <br> <h3 style="text-align: center">Approve Request</h3><br><br>
    ...
    <form class="Approve" method="post" action="">
        <input class="form-control" type="text" name="approve" 
            placeholder="Yes or No" required=""><br>
       
            <input class="form-control" type="text" name="issue" 
        placeholder="Issue Date yyyy-mm-dd" required=""><br>
        
        <input type="text" name="returnd" class="form-control"
            placeholder="Return Date yyyy-mm-ddt" required=""><br>
...
<?php
    ...
    {
        mysqli_query($db,"UPDATE `isseu_book` SET `approve` ='$_POST[approve]', 
        `issue`='$_POST[issue]',`returnd`='$_POST[returnd]' 
...
$res=mysqli_query(db,"SELECT quantity from books 
WHERE bid='$_SESSION[bid]';");
...
while($row=mysqli_fetch_assoc($res))
{
    if($row['quantity']==0)
    {
        ...
        SET status='not-availab' WHERE bid='$_SESSION[bid]';" );
    }
}
?>
