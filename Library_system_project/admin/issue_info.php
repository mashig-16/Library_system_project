    ...
    if(isset($_SESSION['login_user']))
{
    ...
    name,authors,edition,issue,isseu_book.returnd FROM student 
    INNER JOIN isseu_book ON student.username=isseu_book.username 
    INNER JOIN books ON isseu_book.bid=books.bid 
    WHERE isseu_book.approve='yes' ORDER BY `isseu_book`.`returnd` ASC";
     ...
     echo "<tr style='background-color:#89c6cb; '>";
      ...
      {  
         ...
         if($d>$row['returnd'])
         {
          ...
          mysqli_query($db,"UPDATE isseu_book SET approve='$var' 
          WHERE `returnd`='$row[returnd]'and approve='yes' limit $c;");
